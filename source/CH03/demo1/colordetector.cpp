...
cv::Mat CColorDetector::Process(const cv::Mat &image) 
{
    ...
    // same size as input image, but 1-channel
      m_iResult.create(image.size(),CV_8U);

      // Converting to Lab color space
      if (m_bUseLab)
          cv::cvtColor(image, m_iConverted, CV_BGR2Lab);

      // get the iterators
      cv::Mat_<cv::Vec3b>::const_iterator it= image.begin<cv::Vec3b>();
      cv::Mat_<cv::Vec3b>::const_iterator itend= image.end<cv::Vec3b>();
      cv::Mat_<uchar>::iterator itout= m_iResult.begin<uchar>();

      // get the iterators of the converted image
      if (m_bUseLab) {
          it = m_iConverted.begin<cv::Vec3b>();
          itend = m_iConverted.end<cv::Vec3b>();
      }
      ...
      for ( ; it!= itend; ++it, ++itout) {
        ...
        // process each pixel ---------------------
          ...
          if (GetDistanceToTargetColor(*it)<m_nMinDist) {
